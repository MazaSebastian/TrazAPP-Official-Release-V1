...
async function sendTelegram(title: string, message: string) {
    ...
    const CHAT_IDS_RAW = Deno.env.get("TELEGRAM_CHAT_IDS"); // Comma-separated: "870522507,1692599686"

    if (!BOT_TOKEN || !CHAT_IDS_RAW) {
        console.warn("TELEGRAM_BOT_TOKEN or TELEGRAM_CHAT_IDS not configured in Supabase Secrets. Skipping alert.");
        return;
    }
